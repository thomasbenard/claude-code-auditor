...
This is a documentation-only project: a 12-chapter reference guide about Claude Code, organized as markdown files with an `index.md` table of contents. There is no build system, no tests, and no code -- only prose in markdown.

## Structure

- `index.md` -- Table of contents linking all chapters, quick reference table
- `01-introduction.md` through `12-references.md` -- Chapters in reading order
...
- Each chapter ends with a `Next: [Chapter Name](filename.md)` link (except chapter 12 which links back to index)
